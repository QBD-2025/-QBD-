...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const introEndFrameNum = 240;
  const introTotalImages = Math.floor((introEndFrameNum - introStartFrameNum) / framesPerStep) + 1;

  const framePath = '/media/stuart_octavio_libro/frame-';
  const frameExtension = '.webp';

  function getFramePath(frameNumber) {
    const paddedNumber = String(frameNumber).padStart(0, '0'); // 6 dígitos si tus imágenes tienen 6 ceros delante
