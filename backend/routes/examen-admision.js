...
router.get('/examen-exani', isAuthenticated, async (req, res) => {
    try {
        ...
        for (const materia of materias) {
            const [preguntas] = await db.query(
                `SELECT p.id_pregunta, p.id_materia, p.pregunta, p.retroalimentacion, p.puntos
                 FROM pregunta p
                 WHERE p.id_materia = ?
                   AND (
                     SELECT COUNT(*)
        ...
        const [topGlobal] = await db.query(`
            SELECT u.username, u.apodo, u.puntos
            FROM usuario u
...
router.post('/resultados_admision', isAuthenticated, async (req, res) => {
    try {
        const { respuestas, fecha_inicio_str, todosLosIds } = req.body;
        const id_usuario = req.session.user?.id_usuario;

        if (!id_usuario) return res.status(400).send('Falta id_usuario en la peticion.');
        ...
        for (const pregunta of preguntasCompletas) {
            ...
            if (respuestasUsuario[pregunta.id_pregunta] !== undefined) {
                ...
                textoSeleccionado = respuestaSeleccionada ? respuestaSeleccionada.respuesta : 'Invalida';
        ...
        const porcentaje = puntosMaximos > 0
            ? parseFloat(((puntosObtenidos / puntosMaximos) * 100).toFixed(2))
            : 0;
        ...
        console.error('Error generando resultados de admision:', error);
