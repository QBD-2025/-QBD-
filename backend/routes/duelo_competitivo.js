...
// ================================================================
// AGREGAR AL FINAL DE duelo_competitivo.js (ANTES DE module.exports)
// ENDPOINTS DE DEBUG PARA DIAGNOSTICAR PROBLEMAS
// ================================================================

// ================================================================
// AGREGAR ESTOS ENDPOINTS AL FINAL DE duelo_competitivo.js
// (ANTES DE module.exports = router;)
// ================================================================

// ✅ ENDPOINT: Ver TODAS las salas activas (pendientes y espera)
router.get('/api/debug/salas-activas', (req, res) => {
    if (!req.session.user) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    const resumen = {
        ...
        total_pendientes: salasPendientes.size,
        total_espera: salasEspera.size,
        salas_pendientes: [],
        salas_espera: []
    };

    // Listar salas pendientes
    for (const [salaId, sala] of salasPendientes.entries()) {
        resumen.salas_pendientes.push({
            salaId,
            estado: sala.estado,
            tipo: sala.tipo,
            retador: sala.retador || sala.idRetador,
            ...
            jugadoresAceptados: sala.jugadoresAceptados?.size || 0,
            timestamp: sala.timestamp,
            edadSegundos: Math.floor((Date.now() - sala.timestamp) / 1000)
        });
    }

    // Listar salas espera
    for (const [salaId, sala] of salasEspera.entries()) {
        resumen.salas_espera.push({
            salaId,
            estado: sala.estado,
            tipo: sala.tipo,
            retador: sala.retador || sala.idRetador,
    ...
    console.log('[DEBUG SALAS]: Consultado por:', req.session.user.username);
    res.json(resumen);
});

// ✅ ENDPOINT: Ver notificación específica (con verificación de sala)
router.get('/api/debug/notif/:idNotificacion', async (req, res) => {
    if (!req.session.user) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        ...
        const info = {
            ...
            destinatario: notificacion.id_usuario_destinatario,
            remitente: notificacion.id_usuario_remitente,
            fecha_creacion: notificacion.fecha_creacion,
            leido: notificacion.leido,
            extra_data_length: notificacion.extra_data?.length || 0,
            salaId_extraido: salaId,
            sala_verificacion: null
        };
        ...
        if (salaId) {
            ...
            for (const [key, value] of salasPendientes.entries()) {
                if (key === salaId || key.toLowerCase() === salaId.toLowerCase()) {
                    salaEncontrada = value;
                    ubicacion = 'salasPendientes';
                    break;
                }
            }

            if (!salaEncontrada) {
                for (const [key, value] of salasEspera.entries()) {
                    if (key === salaId || key.toLowerCase() === salaId.toLowerCase()) {
                        salaEncontrada = value;
                        ubicacion = 'salasEspera';
                        break;
                    }
                }
            }

            info.sala_verificacion = {
                salaId,
                existe: !!salaEncontrada,
                ubicacion: ubicacion,
                estado: salaEncontrada?.estado || null,
                tipo: salaEncontrada?.tipo || null,
                edadSegundos: salaEncontrada ? Math.floor((Date.now() - salaEncontrada.timestamp) / 1000) : null
            };
        }
        ...
        res.status(500).json({ error: 'Error consultando notificación' });
    }
});

// ✅ ENDPOINT: Simular creación de sala (para testing)
router.post('/api/debug/test-crear-sala', (req, res) => {
    if (!req.session.user || req.session.user.id_tp_usuario < 2) {
        return res.status(403).json({ error: 'No autorizado - requiere permisos de editor' });
    }

    console.log('[DEBUG TEST]: Simulando creación de sala...');

    try {
        // Verificar que la función existe
        if (typeof global.crearSalaPendienteBD !== 'function') {
            return res.status(500).json({
                error: 'crearSalaPendienteBD no disponible',
                tipo: typeof global.crearSalaPendienteBD
            });
        }

        const io = req.app.get('io');
        if (!io) {
            return res.status(500).json({ error: 'Socket.IO no disponible' });
        }

        // Crear sala de prueba
        const idRemitente = req.session.user.id_usuario;
        const idDestino = req.body.idDestino || 999; // Usuario dummy

        console.log('[DEBUG TEST]: Creando sala de prueba...');
        console.log('[DEBUG TEST]: Remitente:', idRemitente);
        console.log('[DEBUG TEST]: Destino:', idDestino);
        ...
        console.log('[DEBUG TEST]: Sala creada:', salaId);
        ...
        console.log('[DEBUG TEST]: En salasPendientes:', enPendientes);
        console.log('[DEBUG TEST]: En salasEspera:', enEspera);

        // Esperar 500ms y verificar de nuevo
        setTimeout(() => {
            const todaviaEnPendientes = salasPendientes.has(salaId);
            const todaviaEnEspera = salasEspera.has(salaId);

            console.log('[DEBUG TEST]: Después de 500ms:');
            console.log('[DEBUG TEST]: En salasPendientes:', todaviaEnPendientes);
            console.log('[DEBUG TEST]: En salasEspera:', todaviaEnEspera);

            if (!todaviaEnPendientes && !todaviaEnEspera) {
                console.error('[DEBUG TEST]: ⚠️ SALA SE PERDIÓ DESPUÉS DE 500ms');
            }
        }, 500);
        ...
        res.json({
            ...
            verificacion_inmediata: {
                en_pendientes: enPendientes,
                en_espera: enEspera
            },
            mensaje: 'Sala de prueba creada. Revisa los logs del servidor.'
        });
...
// ✅ ENDPOINT: Limpiar sala específica
router.delete('/api/debug/limpiar-sala/:salaId', (req, res) => {
    if (!req.session.user || req.session.user.id_tp_usuario < 2) {
        return res.status(403).json({ error: 'No autorizado' });
    }
    ...
    console.log('[DEBUG LIMPIAR]: Limpiando sala:', salaId);
    ...
    for (const [key, value] of salasPendientes.entries()) {
        if (key === salaId || key.toLowerCase() === salaId.toLowerCase()) {
            ...
            salasPendientes.delete(key);
            eliminada = true;
            console.log('[DEBUG LIMPIAR]: Eliminada de salasPendientes:', key);
            break;
        }
    }
    ...
    for (const [key, value] of salasEspera.entries()) {
        if (key === salaId || key.toLowerCase() === salaId.toLowerCase()) {
            if (value.timeoutId) clearTimeout(value.timeoutId);
            salasEspera.delete(key);
            eliminada = true;
            console.log('[DEBUG LIMPIAR]: Eliminada de salasEspera:', key);
            break;
        }
    }
    ...
    res.json({
        success: eliminada,
        salaId,
        mensaje: eliminada ? 'Sala eliminada' : 'Sala no encontrada'
    });
});

// ✅ ENDPOINT: Ver estado completo del sistema
router.get('/api/debug/estado-sistema', (req, res) => {
    if (!req.session.user) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    const estado = {
        ...
        socket_io: {
            disponible: !!io,
            sockets_conectados: io ? io.engine.clientsCount : 0
        },
        funciones_globales: {
        ...
        maps: {
            salasPendientes: {
                existe: global.salasPendientes instanceof Map,
                size: global.salasPendientes?.size || 0
            },
            salasEspera: {
                existe: global.salasEspera instanceof Map,
                size: global.salasEspera?.size || 0
            },
            usuariosConectados: {
                existe: global.usuariosConectados instanceof Map,
                size: global.usuariosConectados?.size || 0
            }
        },
        usuario_actual: {
            id: req.session.user.id_usuario,
            username: req.session.user.username,
            tipo: req.session.user.id_tp_usuario
        }
    };
...
console.log('[DEBUG ENDPOINTS]: ✅ Endpoints de debug registrados');
console.log('[DEBUG ENDPOINTS]: Disponibles:');
console.log('  - GET  /api/debug/salas-activas');
console.log('  - GET  /api/debug/notif/:id');
console.log('  - POST /api/debug/test-crear-sala');
console.log('  - DELETE /api/debug/limpiar-sala/:id');
console.log('  - GET  /api/debug/estado-sistema');
router.get('/api/debug/verificar-sala/:salaId', (req, res) => {
    const { salaId } = req.params;
    ...
    if (global.salasPendientes?.has(salaId)) {
        resultado.encontrada_en.push('global.salasPendientes');
        resultado.datos = global.salasPendientes.get(salaId);
    }
    
    if (global.salasEspera?.has(salaId)) {
        resultado.encontrada_en.push('global.salasEspera');
    }
