    ...
    let tiempoRestante = 3600; // 60 MINUTOS
    let intervaloCronometro = null;
    let mostrarSoloNoRespondidas = true;
    let timerAdvertencia = null;

    // =================================================================
    // FUNCIONES PRINCIPALES (INICIO Y NAVEGACIÓN)
    // =================================================================
    ...
    document.getElementById('btnIniciarExamen').addEventListener('click', comenzarExamen);
    ...
    // =================================================================
    // LÓGICA DE LA LISTA DE PREGUNTAS (PANEL DERECHO)
    // =================================================================
    ...
    function actualizarPendientesUI() {
        ...
        estadoPreguntas.forEach((estado, i) => {
            ...
            if (!mostrarSoloNoRespondidas || esNoRespondida) {
                ...
                let icono = '⚪️';
                const materiaPregunta = preguntas[i].dataset.materia || '';
                let textoEstado = `Pregunta ${i + 1} (${materiaPregunta})`;
    ...
    window.cerrarAviso = function() { document.getElementById("avisoTiempo").classList.add("oculto"); }
    function mostrarAviso(mensaje) { document.getElementById("mensajeAviso").textContent = mensaje; document.getElementById("avisoTiempo").classList.remove("oculto"); }
    ...
    function finalizarExamen() {
        ...
        const todosLosIds = Array.from(preguntas).map(p => p.dataset.idPregunta);
        document.getElementById('inputTodosLosIds').value = JSON.stringify(todosLosIds);
