        ...
        jugadores.forEach(jugador => {
          ...
          item.innerHTML = `
            ...
            <div class="actions">
              ...
              <button class="btn-challenge" data-id="${jugador.id}">Enfrentar ⚔️</button>
            </div>
          `;
    ...
    function setErrors(n) {
      ...
      errorsBadge.style.background = errors ? '#fee2e2' : '#e5e7eb';
    }
    ...
    function clearPathVisual(dragPath) {
      if (dragPath) {
    ...
    function validateDrag() {
      ...
      if (palabraEncontrada) {
        ...
        if (confrontationMode) {
          ...
          setTimeout(() => {
            dragPath.forEach(p => p.el.classList.remove('error'));
          }, 500);
        } else {
          setErrors(errors + 1);
    ...
    function iniciarEnfrentamiento(data) {
      ...
      const myBoardEl = document.getElementById('my-board-grid');
      const opponentBoardEl = document.getElementById('opponent-board-grid');
      renderBoard(myBoardEl, miData.tablero, true);
      renderBoard(opponentBoardEl, oponenteData.tablero, false);
    ...
    function mostrarResultados(data) {
      ...
      const tituloResultados = document.getElementById('results-title');
      tituloResultados.textContent = data.razon || "Resultados del Enfrentamiento";
      
      if (data.ganador) {
        if (data.ganador.id === data.player1.id) {
          document.getElementById('result-player1').classList.add('winner');
        } else {
          document.getElementById('result-player2').classList.add('winner');
        }
      }
    }
    ...
    // Cuando esperamos oponente
    socket.on('sopa:esperandoOponente', () => {
      console.log('⏳ [VOTACIÓN] Recibido: sopa:esperandoOponente');
      ...
      console.log('✅ [VOTACIÓN] Panel mostrado en estado de espera');
    });

    // Cuando ambos jugadores están listos para iniciar votación
    socket.on('sopa:iniciarVotacion', async () => {
      console.log('🗳️ [VOTACIÓN] Recibido: sopa:iniciarVotacion');
      ...
      console.log('✅ [VOTACIÓN] Panel de votación mostrado');
      console.log('📋 [VOTACIÓN] Categorías cargadas:', confrontationCategoriaSelect.options.length);
      
      mostrarMensaje('Sistema', '¡Oponente conectado! Pueden proponer una categoría para jugar.');
    });

    // Botón para proponer categoría
    proposeCategoryBtn.addEventListener('click', () => {
      ...
      console.log('📤 [PROPUESTA] Enviando propuesta:', { salaId, idMateria, materiaTexto });
      ...
      areaProponerDiv.style.display = 'none';
      console.log('✅ [PROPUESTA] Propuesta enviada');
    });

    // Cuando llega una nueva propuesta
    socket.on('sopa:nuevaPropuesta', ({ proponente, materiaTexto }) => {
      console.log('📥 [PROPUESTA] Recibida propuesta de:', proponente, '- Categoría:', materiaTexto);
      votingPanel.style.display = 'block';
      propuestaActualDiv.innerHTML = `<p><strong>${proponente}</strong> ha propuesto: <strong>${materiaTexto}</strong></p>`;
      areaProponerDiv.style.display = 'none';
      
      // Si yo NO soy el proponente, muestro los botones de votación
      if (proponente !== usuario.username) {
        areaVotarDiv.style.display = 'flex';
        console.log('🗳️ [PROPUESTA] Mostrando botones de votación (no soy el proponente)');
      } else {
        areaVotarDiv.style.display = 'none';
        console.log('⏳ [PROPUESTA] Esperando voto (soy el proponente)');
      }
    });

    // Botones de aceptar/rechazar
    aceptarCategoriaBtn.addEventListener('click', () => {
      console.log('✅ [VOTO] Usuario acepta la categoría');
    ...
    rechazarCategoriaBtn.addEventListener('click', () => {
      console.log('❌ [VOTO] Usuario rechaza la categoría');
    ...
    socket.on('sopa:propuestaRechazada', ({ votante }) => {
      console.log('❌ [PROPUESTA] Rechazada por:', votante);
    ...
    socket.on('sopa:enfrentamientoIniciado', (data) => {
      console.log('¡Enfrentamiento iniciado! Datos recibidos:', data);

      if (!data || !data.player1 || !data.player2 || !data.palabras) {
        console.error("Error: datos del servidor incompletos.");
    ...
    socket.on('sopa:enfrentamientoFinalizado', (data) => {
      ...
      const tituloResultados = document.getElementById('results-title');
      if (data.ganador) {
        tituloResultados.textContent = `¡El ganador es ${data.ganador.username}!`;
        if(data.ganador.id === data.player1.id) {
          document.getElementById('result-player1').classList.add('winner');
        } else {
          document.getElementById('result-player2').classList.add('winner');
        ...
        tituloResultados.textContent = "¡Es un empate!";
      }
      
      resultsScreenEl.style.display = 'flex';
    });

    socket.on('sopa:oponenteDesconectado', ({ ganador }) => {
      document.removeEventListener('mouseup', endDrag);
    ...
    socket.on('sopa:revanchaSolicitada', ({ solicitante }) => {
      ...
      if (solicitante.id !== usuario.id_usuario) {
        rematchStatus.innerHTML = `
          ...
          <div class="vote-buttons" style="margin-top: 10px; display: flex; gap: 10px; justify-content: center;">
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      console.log('📍 [INIT] Sala ID:', salaId);
      console.log('👤 [INIT] Usuario:', usuario);
      console.log('🎮 [INIT] Modo:', esEnfrentamiento ? 'ENFRENTAMIENTO' : 'INDIVIDUAL');

      // PASO 1: Unirse al chat (SIEMPRE)
      socket.emit('chat:unirse', { salaId, usuario: usuario.username });
      mostrarMensaje('Sistema', `¡Bienvenido, ${usuario.username}!`);
      console.log('💬 [CHAT] Unido a sala:', salaId);

      if (esEnfrentamiento) {
        // --- MODO ENFRENTAMIENTO ---
        console.log('⚔️ [ENFRENTAMIENTO] Iniciando modo enfrentamiento...');
        confrontationMode = true;
        normalModeEl.style.display = 'none';
        confrontationModeEl.style.display = 'block';
        
        if(document.getElementById('jugadores-panel')) {
          document.getElementById('jugadores-panel').style.display = 'none';
        }
        
        confrontationMessageEl.textContent = "Buscando oponente...";
        console.log('🔌 [SOCKET] Emitiendo sopa:unirseEnfrentamiento...');
        
        socket.emit('sopa:unirseEnfrentamiento', { salaId, usuario });
        console.log('✅ [SOCKET] Evento sopa:unirseEnfrentamiento emitido con datos:', { salaId, usuario });

      } else {
        // --- MODO INDIVIDUAL ---
        console.log('🎯 [INDIVIDUAL] Iniciando modo individual...');
        ...
        console.log('✅ [INDIVIDUAL] Unido a sala individual');
      }
    });