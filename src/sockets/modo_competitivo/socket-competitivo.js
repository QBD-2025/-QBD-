...
// ✅ FUNCIÓN AUXILIAR MEJORADA: Verificar e iniciar duelo si ambos están listos
async function verificarEIniciarDuelo(salaId, io) {
    ...
    try {
        ...
        // ✅ NO ELIMINAR LAS SALAS TODAVÍA - mantenerlas hasta que el duelo esté completamente iniciado
...
module.exports = (io, socket) => {
    ...
    socket.on('duelo:buscar:carrera', async ({ user, dificultad }) => {
        try {
            ...
            if (count === 0) {
                return socket.emit('duelo:error:sinCarrera', { mensaje: 'Debes registrar una carrera en tu perfil.' });
            }
            ...
            if (!pool.some(p => p.userId === user.id_usuario)) {
                pool.push({ userId: user.id_usuario, user, socketId: socket.id, dificultad });
                console.log(`${user.username} en cola Carrera ${dificultad}. Total: ${pool.length}`);
                buscarPareja(pool, 'carrera');
            }
        } catch (error) {
            console.error("Error verificando carrera:", error);
        }
    });
    ...
    socket.on('duelo:buscar:general', (user) => {
        if (!poolGeneral.some(p => p.userId === user.id_usuario)) {
            ...
            console.log(`${user.username} en cola General. Total: ${poolGeneral.length}`);
    ...
    socket.on('duelo:clienteListo', async ({ salaId, userId }) => {
        ...
        if (!duelo.jugadores[userId] || !duelo.jugadores[userId].username) {
            ...
            try {
                const [userData] = await db.query('SELECT id_usuario, username, foto_perfil FROM usuario WHERE id_usuario = ?', [userId]);
