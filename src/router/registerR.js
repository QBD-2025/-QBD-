...
router.post('/register', async (req, res) => {
    ...
    try {
        const [existingUsers] = await req.pool.query('SELECT id FROM usuario WHERE email = ?', [email]);
        ...
        await req.pool.query(
            'INSERT INTO usuario (username, email, password, verificado, token, token_expira) VALUES (?, ?, ?, ?, ?, ?)',
            [username, email, hashedPassword, 0, token, tokenExpires]
        );
