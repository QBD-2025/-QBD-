...
router.get('/presentacion', (req, res) => {
    ...
    res.render('presentacion', { user: req.session.user, layout: 'main', title: 'Bienvenida' });
});
...
router.get('/cambiar-contrasena', async (req, res) => {
    ...
    try {
        const [rows] = await req.pool.query(
            'SELECT id_usuario FROM usuario WHERE token_reseteo = ? AND token_reseteo_expira > NOW()',
            [token]
        );
